...
if __name__ == "__main__":
	...
	WTWorkDir='../../WTLeptin_LR/RMSDEvolution'
	MTWorkDir='../../L72SLeptin_LR/RMSDEvolution'
	...
	for i in range(1,6):
		...
		# FileAB
		with open(f"{WTWorkDir}/Rep{i}/fileAB.xvg", 'r') as f:
			FileABContent = f.readlines()
            
		FileABContent=[[float(i.strip().split()[0])/1000,float(i.strip().split()[1])/10] for i in FileABContent if not (i.startswith('#') or i.startswith('@'))]
		WTFileABContents.append(FileABContent)


		# FileCD
		with open(f"{WTWorkDir}/Rep{i}/fileCD.xvg", 'r') as f:
			FileCDContent = f.readlines()
            
		FileCDContent=[[float(i.strip().split()[0])/1000,float(i.strip().split()[1])/10] for i in FileCDContent if not (i.startswith('#') or i.startswith('@'))]
		WTFileCDContents.append(FileCDContent)      
    
		# FileEF
		with open(f"{WTWorkDir}/Rep{i}/fileEF.xvg", 'r') as f:
			FileEFContent = f.readlines()
            
		FileEFContent=[[float(i.strip().split()[0])/1000,float(i.strip().split()[1])/10] for i in FileEFContent if not (i.startswith('#') or i.startswith('@'))]        
	...
	for i in range(1,6):
		...
		# FileAB
		with open(f"{MTWorkDir}/Rep{i}/fileAB.xvg", 'r') as f:
			FileABContent = f.readlines()
		...
		# FileCD
		with open(f"{MTWorkDir}/Rep{i}/fileCD.xvg", 'r') as f:
			FileCDContent = f.readlines()
		...
		# FileEF
		with open(f"{MTWorkDir}/Rep{i}/fileEF.xvg", 'r') as f:
			FileEFContent = f.readlines()
	...
	for j in range(5):
		...
		# Set y axis limit
		globals()['ax%s' % j].set_ylim([0.0, 0.51])
	...
	for j in range(5):
		...
		# Set y axis limit
		globals()['ax%sx' % j].set_ylim([0.0, 0.51])
		# Set legend
		...
		ax0.set_ylabel('RMSD (nm)',size=10, labelpad=20)
		ax1.set_ylabel('RMSD (nm)',size=10, labelpad=20)
		ax2.set_ylabel('RMSD (nm)',size=10, labelpad=20)
		ax3.set_ylabel('RMSD (nm)',size=10, labelpad=20)
		ax4.set_ylabel('RMSD (nm)',size=10, labelpad=20)
	...
	# Save figure
	fig.tight_layout()
	plt.savefig(f"RMSDEvolution.png",dpi=600,bbox_inches='tight')
	plt.savefig(f"RMSDEvolution.eps",dpi=600,bbox_inches='tight')
