...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description = 'Plotting rmsd values distribution')
	parser.add_argument("--WTSampledValue", type=validate_file, help="File containing sampled values of WT structure, displayed in nm (.xvg)", required=True)
	parser.add_argument("--MTSampledValue", type=validate_file, help="File containing sampled values of MT structure, displayed in nm (.xvg)", required=True)
	...
	with open(WTVolumeValues, 'r') as f:
		WTVolumeValuesFileContent = f.readlines()

	WTVolumeValuesFileContent=[float(i.strip().split()[1]) for i in WTVolumeValuesFileContent if not (i.startswith('#') or i.startswith('@'))]

	# MT Sampled volume values
	with open(MTVolumeValues, 'r') as f:
		MTVolumeValuesFileContent = f.readlines()

	MTVolumeValuesFileContent=[float(i.strip().split()[1]) for i in MTVolumeValuesFileContent if not (i.startswith('#') or i.startswith('@'))]
	...
	plt.tight_layout()
	plt.xlabel('RMSD (nm)',labelpad=10,fontsize=15)
